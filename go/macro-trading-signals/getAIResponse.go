...
func getInference(newsArticle string, modelToUse string) (TradeSignal, error) {
	...
	promptResponseDefinition := "Trade signal should be a json object having only the following keys: tradeDirection, tickerSymbol, buyPrice, sellPrice. The trade signal is only for merger and acquisition news so if you are not confident that the news can be categorized as such, set all values to `unsure` in the json response. The tradeDirection should be 'buy' if it is positive news for the company being acquired or 'sell' if it is negative news. The ticker symbol is only for the symbol of the company being acquired. All values in the JSON will be a string. If you are less than 50 percent confident about a value or there is no information available, enter 'unsure' as the string. Your output should only be in json format."
	// Trade on all news
